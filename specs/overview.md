...
- マスタ、履歴、売上は物理削除しない。
- 登録済み売上は過去の取引証跡として扱い、入力間違いがあっても売上ヘッダーや明細を直接更新しない。
- 売上訂正は取消と再登録で表現し、元売上、取消売上、再登録売上の関係を追跡できるようにする。
- 外部キーの削除動作は `Restrict` を基本とする。
- `CreatedAt` はアプリケーション側で UTC 現在日時を設定する。
- DB に保存する監査用の日時は UTC を標準とする。
- 業務上の「現在日付」はアプリケーション側で JST の業務日付として解決する。
- `SalesDate` と `ValidFrom` はタイムゾーン付きの瞬間ではなく、JST 業務カレンダー上の日付として扱い、保存時は日付部分に正規化する。
- 履歴適用判定では、アプリケーション側で決めた業務日付を `ValidFrom` と比較する。
...
- 登録済み売上は原則更新しない。
- 入力間違いは元売上の取消で表現する。
- 正しい売上が必要な場合は、取消後に正しい売上を新規登録する。
- 取消理由、取消日時、取消実行者を残す。
- 通常業務の一覧では有効売上を中心に表示し、監査用途では取消済み売上や取消売上も確認できるようにする。

詳細は `specs/sales-correction.md` に定義する。
